...
public class PacMan1 extends PacMan{
	...
	public int move(){
		if (inBlock()){
			...
			while ((random == Constant.LEFT && _prevDir == Constant.RIGHT) || (random == Constant.RIGHT && _prevDir == Constant.LEFT) || (random == Constant.UP && _prevDir == Constant.DOWN) || (random == Constant.DOWN && _prevDir == Constant.UP) || !isValidMove(random)){
	...
	public boolean isValidMove(int direction){
		...
		switch (direction){
			case Constant.LEFT:
				if (col-1 >=0 && map[row][col-1] != Constant.WALL || (row == 14 && col == 0)){
					return true;
				}
				break;
			case Constant.RIGHT:
				if (col+1 < map.length && map[row][col+1] != Constant.WALL || (row == 14 && col == 27)){
					return true;
				}
				break;
			case Constant.UP:
				if (row-1 >=0 && map[row-1][col] != Constant.WALL){
					return true;
				}
				break;
			case Constant.DOWN:
				if (row+1 < map.length && map[row+1][col] != Constant.WALL){
					return true;
				}
		}
		return false;
	...
	public PacMan1(ProxyBoard board){super(board);}
